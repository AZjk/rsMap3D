...
from rsMap3D.gui.rsmap3dsignals import (
    CLEAR_RENDER_WINDOW_SIGNAL,
    DONE_LOADING_SIGNAL,
    RENDER_BOUNDS_SIGNAL,
...
class ScanForm(qtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.scanList.itemClicked[qtWidgets.QListWidgetItem].connect(self._scanSelected)
    ...
    def addAvailableScanTypes(self):
        ...
        for scanType in self.dataSource.getAvailableScanTypes():
            ...
            typeCheckBox.stateChanged[int].connect(self.availableScanTypesChanged)
    ...
    def availableScanTypesChanged(self, state):
        ...
        for scanType in scanTypes:
            if isinstance(scanType, qtWidgets.QWidget):
                self.dataSource.setScanTypeUsed(scanType.text(), scanType.isChecked())
    ...
    def loadScanFile(self, dataSource):
        ...
        self.detail.setColumnCount(7 + len(dataSource.getReferenceNames()))
        labels = []
        labels.append("Use Image")
        labels.extend(dataSource.getReferenceNames())
        labels.extend(["Min qx", "Max qx", "Min qy", "Max qy", "Min qz", "Max qz"])
    ...
    def removeAvailableScanTypes(self):
        ...
        for scanType in scanTypes:
            if isinstance(scanType, qtWidgets.QWidget):
                scanType.stateChanged[int].disconnect(self.availableScanTypesChanged)
        ...
        for scan in self.dataSource.getAvailableScans():
            ...
            for i in range(0, len(minx) - 1, int(step)):
                if imageToBeUsed[scan][i]:
                    self.renderBoundsSignal[object].emit((minx[i], maxx[i], miny[i], maxy[i], minz[i], maxz[i]))
    ...
    def showAngles(self, scanNo):
        ...
        if self.initialSelectionMade:
            self.detail.itemChanged[qtWidgets.QTableWidgetItem].disconnect(self.checkItemChanged)
        ...
        for angle in angles:
            ...
            row += 1
        self.detail.itemChanged[qtWidgets.QTableWidgetItem].connect(self.checkItemChanged)
    ...
    def showQs(self, scan):
        ...
        for _value in xmin:
            if imageToBeUsed[scan][row]:
                ...
                self.renderBoundsSignal[object].emit((xmin[row], xmax[row], ymin[row], ymax[row], zmin[row], zmax[row]))
        ...
        scanXmin, scanXmax, scanYmin, scanYmax, scanZmin, scanZmax = self.dataSource.findScanQs(
        ...
        self.renderBoundsSignal[object].emit((scanXmin, scanXmax, scanYmin, scanYmax, scanZmin, scanZmax))
        self.showRangeBounds[object].emit((scanXmin, scanXmax, scanYmin, scanYmax, scanZmin, scanZmax))
        self.detail.itemChanged[qtWidgets.QTableWidgetItem].connect(self.checkItemChanged)
        self.initialSelectionMade = True
