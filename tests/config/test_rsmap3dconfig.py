...
BAD_FILE_DIRECTORY = os.path.join(THIS_DIR, "../fixtures/problemFilesForTesting/")
...
class Test(unittest.TestCase):
    ...
    def testGetMaxImageMemoryNoMaxImageMem(self):
        config2 = RSMap3DConfig(fileName=BAD_FILE_DIRECTORY + "rsmapConfigNoMaxImageMemory.xml")
        self.assertRaises(RSMap3DException, config2.getMaxImageMemory)

    def testGetMaxImageMemoryEmptyMaxImageMem(self):
        config3 = RSMap3DConfig(fileName=BAD_FILE_DIRECTORY + "rsmapConfigEmptyMaxImageMemory.xml")
        self.assertRaises(RSMap3DException, config3.getMaxImageMemory)

    def testGetMaxImageMemoryBadCharacters(self):
        config4 = RSMap3DConfig(fileName=BAD_FILE_DIRECTORY + "rsmapConfigMaxImageMemBadCharacters.xml")
