...
class XPCSSpecScanFileForm(SpecXMLDrivenFileForm):
    ...
    def _browseForReplacePathWith(self):
        if self.replacePathWithTxt.text() == EMPTY_STR:
            fileName = qtWidgets.QFileDialog.getExistingDirectory(None, XPCS_FILE_DIALOG_TITLE)
        else:
            fileDirectory = os.path.dirname(str(self.replacePathWithTxt.text()))
            fileName = qtWidgets.QFileDialog.getExistingDirectory(None, XPCS_FILE_FILTER, dir=fileDirectory)
    ...
    def checkOkToLoad(self):
        ...
        superOk = super().checkOkToLoad()
        self.isXpcsFileNameOK()
    ...
    def _createDataBox(self):
        ...
        label1 = qtWidgets.QLabel(
            "<b>The following correct the file path if it has"
    ...
    def getDataSource(self):
        if self.getOutputType() == self.SIMPLE_GRID_MAP_STR:
            ...
            self.transform = PoleMapTransform3D(projectionDirection=self.fileForm.getProjectionDirection())
        else:
            self.transform = None
        ...
        self.dataSource = XPCSSpecDataSource(
    ...
    def isReplacePathWithNameOK(self):
        return os.path.isdir(self.replacePathWithTxt.text()) or (self.replacePathWithTxt.text() == EMPTY_STR)
    ...
    def _pathToReplaceChanged(self):
        if self.isXpcsFileNameOK():
            ...
            message.warning(self, WARNING_STR, "The path To replace is invalid")
    ...
    def _replacePathWithChanged(self):
        if self.isReplacePathWithNameOK():
            ...
            message = qtWidgets.QMessageBox()
            message.warning(self, WARNING_STR, "The replace Path with is invalid")
