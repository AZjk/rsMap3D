...
class S34HDFEScanFileForm(AbstractImagePerFileView):
    ...
    def checkOkToLoad(self):
        ...
        if projFileOK and os.path.isfile(self.detConfigTxt.text()):
            retVal = True
    ...
    def _detConfigChanged(self):
        """ """
        if os.path.isfile(self.detConfigTxt.text()) or self.detConfigTxt.text() == "":
            self.checkOkToLoad()
        #             try:
