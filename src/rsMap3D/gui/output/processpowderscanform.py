...
from rsMap3D.mappers.powderscanmapper import X_COORD_OPTIONS, Y_SCALING_OPTIONS, PowderScanMapper
...
class ProcessPowderScanForm(AbstractOutputView):
    ...
    def _browseForOutputFile(self):
        ...
        if self.outFileNameTxt == EMPTY_STR:
            fileName = str(qtWidgets.QFileDialog.getSaveFileName(None, SAVE_FILE_STR, filter=XYE_FILTER_STR)[0])
        else:
            inFileName = str(self.outFileNameTxt.text())
            fileName = str(
                qtWidgets.QFileDialog.getSaveFileName(None, SAVE_FILE_STR, filter=XYE_FILTER_STR, directory=inFileName)[
                    0
                ]
            )
        if fileName != EMPTY_STR:
            if os.path.exists(os.path.dirname(str(fileName))):
                ...
                message.warning(self, WARNING_STR, "The specified directory does not exist")
            ...
            if not os.access(os.path.dirname(fileName), os.W_OK):
                message = qtWidgets.QMessageBox()
                message.warning(self, WARNING_STR, "The specified file is not writable")
        else:
            ...
            if os.path.exists(os.path.dirname(fileName)):
                ...
                if os.path.dirname(fileName) == EMPTY_STR:
                    ...
                    message.warning(
                        self,
                        WARNING_STR,
                        "The specified directory \n" + str(os.path.dirname(fileName)) + "does not exist",
                    )
    ...
    def runMapper(self, dataSource, transform, gridWriter=None):
        ...
        logger.debug(
            f"Loading PowderScanMapper with xmin {str(self.outMinTxt.text())}, xmax {str(self.outMaxTxt.text())}, xstep {str(self.outStepTxt.text())}"
        )
        self.mapper = PowderScanMapper(
            self.dataSource,
            self.outputFileName,
            transform=transform,
