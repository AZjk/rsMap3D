...
from rsMap3D.gui.rsm3dcommonstrings import BROWSE_STR, SAVE_FILE_STR, WARNING_STR
...
class ProcessXpcsGridLocationForm(AbstractGridOutputForm):
    ...
    def _browseForOutputFile(self):
        ...
        if self.outFileTxt.text() == "":
            fileName = str(qtWidgets.QFileDialog.getSaveFileName(None, SAVE_FILE_STR, filter=self.outFilter)[0])
        else:
            inFileName = str(self.outFileTxt.text())
            fileName = str(
                qtWidgets.QFileDialog.getSaveFileName(None, SAVE_FILE_STR, filter=self.outFilter, directory=inFileName)[
                    0
                ]
            )
        if fileName != "":
    ...
    def runMapper(self, dataSource, transform):
        ...
        if self.outputFileName == "":
            self.outputFileName = os.path.join(
                dataSource.projectDir, f"{dataSource.projectName}{self.gridWriter.FILE_EXTENSION}"
            )
            self.setFileName.emit(self.outputFileName)

        if os.access(os.path.dirname(self.outputFileName), os.W_OK):
            self.mapper = XPCSGridLocationMapper(
                dataSource,
                self.outputFileName,
                nx=nx,
                ny=ny,
