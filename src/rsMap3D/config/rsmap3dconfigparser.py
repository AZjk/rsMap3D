...
class RSMap3DConfigParser(ConfigParser):
    ...
    def __init__(self, fileName="", **kwargs):
        ...
        try:
            if not exists(self.configFile):
                logger.debug("Could not find existing logfile " + str(self.configFile))
                self.buildDefaultConfig()
                try:
                    logger.debug("Try reading debug info from previous XML " + "file ")

                    configFileOld = RSMap3DConfig()
                    self.MaxMemory = configFileOld.getMaxImageMemory()
                    self.set(MEMORY_SECTION_NAME, MAX_IMAGE_MEMORY, str(self.MaxMemory))
                except Exception:
    ...
    def getMPIWorkerHostFile(self, default="worker_hosts"):
        ...
        logger.debug(f"Sections {str((sections,))}")
        try:
            if MPI_SECTION_NAME in sections:
                hostFile = self.get(MPI_SECTION_NAME, MPI_HOST_FILE)
                logger.debug(f"MPI HostFile {str(hostFile)}")
            else:
                logger.warning(
                    "MPI Section not found in app config file\n" + "default section looks like\n" + "[MPI]\n"
                    "HostFile = worker_hosts\n"
                    "WorkerCount = 1\n"
                )
                hostFile = MPI_DEFAULT_HOST
        except Exception as ex:
            logger.error(f"No MPI_HOST_FILE found using defaut; {str(MPI_DEFAULT_HOST)}")
    ...
    def getMPIWorkerCount(self, default=1):
        ...
        try:
            ...
            if MPI_SECTION_NAME in mpiSections:
                ...
                workerCount = self.getint(MPI_SECTION_NAME, MPI_WORKER_COUNT)
