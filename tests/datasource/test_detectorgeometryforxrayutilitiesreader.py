...
class Test(unittest.TestCase):
    def testGetCenterChannelPixel(self):
        config = DetectorGeometryForXrayutilitiesReader(GOOD_FILE_NAME)
        detector = config.getDetectorById("Pilatus")
        centerPix = config.getCenterChannelPixel(detector)
        pix = [210, 85]
        self.assertEqual(centerPix, pix, "getCenterChannelPixel " + str(centerPix) + " vs " + str(pix))
    ...
    def testGetDetectorsNoDetectors(self):
        config = DetectorGeometryForXrayutilitiesReader(BAD_FILE_NO_DETECTOR_LIST)
    ...
    def testGetDetectorByIdNoDetectorList(self):
        config = DetectorGeometryForXrayutilitiesReader(BAD_FILE_NO_DETECTOR_LIST)
        self.assertRaises(DetectorConfigException, config.getDetectorById, DETECTOR_NAME)

    def testGetDetectorByIdNoDetector(self):
        config = DetectorGeometryForXrayutilitiesReader(BAD_FILE_NO_DETECTOR)
        self.assertRaises(DetectorConfigException, config.getDetectorById, DETECTOR_NAME)
