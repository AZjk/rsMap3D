...
def _updateDataSourceProgress(value1, value2):
    logger.info(f"DataSource Progress {value1}/{value2}")


def _updateMapperProgress(value1):
    logger.info(f"Mapper Progress {value1}")
...
def run(config):
    ...
    for scans in scanList1:
        ...
        logger.info(f"specName, specExt: {specName}, {specExt}")
        ...
        logger.info(f"imageToBeUsed {imageToBeUsed}")
        ...
        for zeroBasedIndex in range(numImages):
            ...
            outputFileName = os.path.join(projectDir, specName + ("_N%d.vti" % imageInScan))
