...
class SpecXMLDrivenDataSource(AbstractXrayutilitiesDataSource):
    def __init__(self, projectDir, projectName, projectExtension, instConfigFile, detConfigFile, **kwargs):
        ...
        self.currentDetector = detConfig.getDetectorById(firstDetectorID)
        self.progress = 0
        self.progressInc = 1
        self.progressMax = 1
        self.haltMap = False
    ...
    def findImageQs(self, angles, ub, en):
        ...
        hxrd = xu.HXRD(self.getInplaneReferenceDirection(), self.getSampleSurfaceNormalDirection(), en=en, qconv=qconv)
    ...
    def getReferenceValues(self, scan):
        ...
        try:
            ...
            logger.error("Get Reference Values Scan data missing " + str(e.message))
        return angles
    ...
    def getScanAngles(self, scan, angleNames):
        ...
        try:
            ...
            raise ex
        if len(dataKeys) == 0:
            raise ScanDataMissingException("No Scan Data Found for scan " + scan.scanNum)
        ...
        for i, name in enumerate(angleNames):
            ...
            if v is not None:
                ...
                raise InstConfigException("Could not find angle " + name + " in scan parameters")
            geoAngles[:, i] = v
        return geoAngles
