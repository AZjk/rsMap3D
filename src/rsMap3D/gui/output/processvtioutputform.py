...
class ProcessVTIOutputForm(AbstractOutputView):
    ...
    def _browseForOutputFile(self):
        ...
        if self.outFileTxt.text() == EMPTY_STR:
            fileName = str(qtWidgets.QFileDialog.getSaveFileName(None, SAVE_FILE_STR, filter=VTI_FILTER_STR)[0])
        else:
            inFileName = str(self.outFileTxt.text())
            fileName = str(
                qtWidgets.QFileDialog.getSaveFileName(None, SAVE_FILE_STR, filter=VTI_FILTER_STR, directory=inFileName)[
                    0
                ]
            )
    ...
    def _createDataBox(self):
        ...
        self.outputTypeSelect.currentTextChanged.connect(self._selectedTypeChanged)
        logger.debug(METHOD_EXIT_STR)
        return dataBox
    ...
    def runMapper(self, dataSource, transform, gridWriter=None):
        ...
        if self.outputFileName == "":
            self.outputFileName = os.path.join(dataSource.projectDir, f"{dataSource.projectName}.vti")
            self.setFileName[str].emit(self.outputFileName)
        if os.access(os.path.dirname(self.outputFileName), os.W_OK):
            self.mapper = QGridMapper(
                dataSource,
                self.outputFileName,
                outType,
                nx=nx,
