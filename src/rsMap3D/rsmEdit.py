...
from rsMap3D.config.rsmap3dlogging import LOGGER_DEFAULT, LOGGER_NAME, METHOD_ENTER_STR, METHOD_EXIT_STR
...
if os.path.exists(logConfigFile):
    ...
    try:
        ...
        print(f"In Exception to load dictConfig package {LOGGER_NAME}\n {ex}")
        logging.config.dictConfig(LOGGER_DEFAULT)
    except KeyError as ex:
        print(f"logfile {logConfigFile} was missing or had errant sections {ex.args}")
...
class MainDialog(qtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.processScans = ProcessScansController(parent=self, appConfig=self.appConfig)
        ...
        self.fileForm.loadDataSourceToScanForm.connect(self._loadDataSourceToScanForm)
        ...
        self.scanForm.showRangeBounds[object].connect(self.dataExtentView.showRangeBounds)
        self.scanForm.clearRenderWindow.connect(self.dataExtentView.clearRenderWindow)
        self.scanForm.renderBoundsSignal[object].connect(self.dataExtentView.renderBounds)
        self.processScans.blockTabsForProcess.connect(self._blockTabsForProcess)
        self.processScans.unblockTabsForProcess.connect(self._unblockTabsForProcess)
    ...
    def _setupRanges(self):
        ...
        """
        overallXmin, overallXmax, overallYmin, overallYmax, overallZmin, overallZmax = (
            self.fileForm.dataSource.getOverallRanges()
        )
        self.dataRange.setRanges(overallXmin, overallXmax, overallYmin, overallYmax, overallZmin, overallZmax)
        self._setScanRanges()
    ...
    def _showFileError(self, error):
        ...
        message.warning(self, "Load Scan File Warning", str(error))
