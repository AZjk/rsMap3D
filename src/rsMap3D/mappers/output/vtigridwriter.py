...
class VTIGridWriter(AbstractGridWriter):
    def setFileInfo(self, fileInfo):
        ...
        if (fileInfo is None) or (len(fileInfo) == 0):
            raise ValueError(
                self.whatFunction() + "passed no filename information " + "requires a tuple with six members:\n"
    ...
    def write(self):
        ...
        if self.outputFileName == "":
            writer.SetFileName(VTI_WRITER_MERGE_STR % (self.fileInfo[0], self.fileInfo[1]))
        else:
