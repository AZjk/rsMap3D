...
class AbstractImagePerFileView(AbstractFileView):
    ...
    def _browseForProjectDir(self):
        ...
        if self.projNameTxt.text() == EMPTY_STR:
            fileName = qtWidgets.QFileDialog.getOpenFileName(None, self.fileDialogTitle, filter=self.fileDialogFilter)[
                0
            ]
        else:
            fileDirectory = os.path.dirname(str(self.projNameTxt.text()))
            fileName = qtWidgets.QFileDialog.getOpenFileName(
                None, self.fileDialogTitle, directory=fileDirectory, filter=self.fileDialogFilter
            )[0]
    ...
    def _createControlBox(self):
        ...
        logger.debug(METHOD_ENTER_STR)
        logger.debug(METHOD_EXIT_STR)
        return super()._createControlBox()
    ...
    def _projectDirChanged(self):
        ...
        """
        logger.debug(METHOD_ENTER_STR)
        if os.path.isfile(self.projNameTxt.text()) or self.projNameTxt.text() == EMPTY_STR:
            self.checkOkToLoad()
        else:
            logger.warning("The project directory entered is invalid")
            message = qtWidgets.QMessageBox()
            message.warning(self, WARNING_STR, "The project directory entered is invalid")
        logger.debug(METHOD_EXIT_STR)
