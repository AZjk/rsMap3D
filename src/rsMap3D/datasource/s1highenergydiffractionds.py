...
from rsMap3D.exception.rsmap3dexception import DetectorConfigException, InstConfigException, RSMap3DException
...
class S1HighEnergyDiffractionDS(AbstractXrayutilitiesDataSource):
    ...
    def findImageQs(self, angles, ub, en):
        logger.debug(METHOD_ENTER_STR)
        logger.debug("sampleCircleDirections: " + str(self.sampleCircleDirections))
        logger.debug("detectorCircleDirections: " + str(self.detectorCircleDirections))
        logger.debug("primaryBeamDirection: " + str(self.primaryBeamDirection))
        qconv = xu.experiment.QConversion(
            self.sampleCircleDirections, self.detectorCircleDirections, self.primaryBeamDirection
        )
        logger.debug("en: " + str(en))
        logger.debug("qconv: " + str(qconv))
        logger.debug("inplaneReferenceDirection: " + str(self.sampleInplaneReferenceDirection))
        logger.debug("sampleSurfaceNormalDirection: " + str(self.sampleSurfaceNormalDirection))
        hxrd = xu.HXRD(self.sampleInplaneReferenceDirection, self.sampleSurfaceNormalDirection, en=en, qconv=qconv)
        ...
        logger.debug("pixelDirections: " + str((self.detectorPixelDirection1, self.detectorPixelDirection2)))
        logger.debug(f"distance to detector {self.distanceToDetector:f}, pixel Size {str(self.detectorPixelWidth)}")
        ...
        if imageSize * 4 * numImages <= maxImageMem:
            ...
            if ub is None:
                qx, qy, qz = hxrd.Ang2Q.area(*angleList, roi=roi, Nav=nav)
            else:
                qx, qy, qz = hxrd.Ang2Q.area(*angleList, roi=roi, Nav=nav, UB=ub)

            logger.debug("Before transform qx, qy, qz " + str((qx, qy, qz)))
            qxTrans, qyTrans, qzTrans = self.transform.do3DTransform(qx, qy, qz)
            logger.debug("After transform qx, qy, qz " + str((qxTrans, qyTrans, qzTrans)))
            ...
            for thisPass in range(nPasses):
                ...
                logger.debug("firstImageInPass %d, lastImageInPass %d" % (firstImageInPass, lastImageInPass))
    ...
    def getGeoAngles(self, scan):
        ...
        if self.angleParams[scan][S1_START_POS] != self.angleParams[scan][S1_END_POS]:
            stepSize = (self.angleParams[scan][S1_END_POS] - self.angleParams[scan][S1_START_POS]) / self.angleParams[
                scan
            ][NFRAMES]
            logger.debug(f"range {str(len(range(0, self.angleParams[scan][NFRAMES])))} ")
            for frame in range(0, self.angleParams[scan][NFRAMES]):
                s1Angles[frame] = self.angleParams[scan][S1_START_POS] + frame * stepSize
        else:
            s1Angles = s1Angles * self.angleParams[scan][S1_START_POS]

        if self.angleParams[scan][S2_START_POS] != self.angleParams[scan][S2_END_POS]:
            stepSize = (self.angleParams[scan][S2_END_POS] - self.angleParams[scan][S2_START_POS]) / self.angleParams[
                scan
            ][NFRAMES]
            for frame in range(0, self.angleParams[scan][NFRAMES]):
                s2Angles[frame] = self.angleParams[scan][S2_START_POS] + frame * stepSize
        else:
            s2Angles = s2Angles * self.angleParams[scan][S2_START_POS]

        if self.angleParams[scan][D1_START_POS] != self.angleParams[scan][D1_END_POS]:
            stepSize = (self.angleParams[scan][D1_END_POS] - self.angleParams[scan][D1_START_POS]) / self.angleParams[
                scan
            ][NFRAMES]
            for frame in range(0, self.angleParams[scan][NFRAMES]):
                d1Angles[frame] = self.angleParams[scan][D1_START_POS] + frame * stepSize
        else:
    ...
    def loadDetectorXMLConfig(self):
        logger.debug(METHOD_ENTER_STR)
        try:
            detConfig = DetectorReader.DetectorGeometryForXrayutilitiesReader(self.detConfigFile)
            ...
            if self.detectorROI is None:
                self.detectorROI = [0, self.detectorDimensions[0], 0, self.detectorDimensions[1]]
    ...
    def loadInstrumentXMLConfig(self):
        ...
        try:
            ...
            self.sampleCircleDirections = self.instConfig.getSampleCircleDirections()
            ...
            self.sampleInplaneReferenceDirection = self.instConfig.getInplaneReferenceDirection()
            self.sampleSurfaceNormalDirection = self.instConfig.getSampleSurfaceNormalDirection()
            ...
            self.angleNames = self.instConfig.getSampleCircleNames() + self.instConfig.getDetectorCircleNames()
            ...
            logger.error("Unhandle Exception loading instrument config" + str(ex))
            raise ex
        logger.debug(METHOD_EXIT_STR)
    ...
    def loadSource(self, mapHKL=False):
        ...
        self.parFileName = os.path.join(self.projectDir, self.projectName + self.projectExtension)
        ...
        for scan in self.scans:
            ...
            if self.mapHKL:
                ...
                if self.ubMatrix[scan] is None:
                    raise RSMap3DException("UB matrix " + "not found.")
                else:
                    self.ubMatrix[scan] = None

            # TODO
            energyData = self.parFile.getEnergy(
                [
                    scan,
                ]
            )
            ...
            self.incidentEnergy[scan] = energyData[scan][INCIDENT_ENERGY] * KEV_TO_EV
            # else:
            logging.debug("energyData[scan]: " + str(self.incidentEnergy[scan]))
            _start_time = time.time()
            self.imageBounds[scan] = self.findImageQs(angles, self.ubMatrix[scan], self.incidentEnergy[scan])

        self.fileParams = self.parFile.getFileData(self.scans)
        logger.debug(METHOD_EXIT_STR)

    # @profile
    def rawmap(self, scans, angledelta=None, adframes=None, mask=None):
        if angledelta is None:
            angledelta = [0, 0, 0, 0]
        logger.debug(METHOD_ENTER_STR)
        if mask is None:
            mask_was_none = True
        ...
        if (self.detectorPixelWidth is not None) and (self.distanceToDetector is not None):
            hxrd.Ang2Q.init_area(
        ...
        for scannr in scans:
            ...
            foundIndex = 0
            logger.debug(f"imageDirName {self.imageDirName}")
            ...
            for ind in range(len(angles[:, 0])):
                if imageToBeUsed[scannr][ind] and mask[ind]:
                    ...
                    image = np.empty((self.detectorDimensions[0], self.detectorDimensions[1]), np.float32)
                    ...
                    for i in range(len(angleNames)):
                        logger.debug("appending angles to angle2 " + str(scanAngle1[i][ind]))
        ...
        logger.debug(f"Before hxrd.Ang2Q.area {str(angleList)}")
        if self.ubMatrix[scans[0]] is None:
            qx, qy, qz = hxrd.Ang2Q.area(*angleList, roi=self.detectorROI, Nav=self.numPixelsToAverage)
        else:
            qx, qy, qz = hxrd.Ang2Q.area(
                *angleList, roi=self.detectorROI, Nav=self.numPixelsToAverage, UB=self.ubMatrix[scans[0]]
            )
...
class S1ParameterFile:
    def __init__(self, fileName):
        ...
        with open(fileName) as parFile:
            reader = csv.DictReader(parFile, delimiter=" ", skipinitialspace=True)
            numElements = 0
            ...
            for row in reader:
                ...
                if (
                    (not len(row[FIRST_COL]) == 0)
                    and (not row[FIRST_COL].startswith("#"))
                    and (not row[FIRST_COL] == "")
                ):
                    if len(self.lines) == 0:
                        numElements = len(row)
                    else:
    ...
    def getEnergy(self, lineNums):
        ...
        for lineNum in lineNums:
            ...
            energyData[INCIDENT_ENERGY] = float(self.lines[lineNum - 1][INCIDENT_ENERGY])
