...
try:
    ...
    from pyimm.immheader import getNumberOfImages, readHeader
...
from rsMap3D.datasource.Sector33SpecDataSource import (
    IMAGE_DIR_MERGE_STR,
    LoadCanceledException,
    Sector33SpecFileException,
)
...
class XPCSSpecDataSource(SpecXMLDrivenDataSource):
    def __init__(self, projectDir, projectName, projectExtension, instConfigFile, detConfigFile, **kwargs):
        ...
        if kwargs[PATH_TO_REPLACE] is not None:
            self.pathToReplace = kwargs[PATH_TO_REPLACE]
            logger.debug(f"pathToReplace: {self.pathToReplace}")

        if kwargs[REPLACE_PATH_WITH] is not None:
            self.replacePathWith = kwargs[REPLACE_PATH_WITH]
            logger.debug(f"replacePathWith: {self.replacePathWith}")
    ...
    def loadSource(self, mapHKL=False):
        ...
        try:
            ...
            logger.debug(f"{str(maxScan)} scans")
            ...
            for scan in self.scans:
                if self.cancelLoad:
                    ...
                    try:
                        ...
                        logger.debug(f"Scan {scan} scanType {self.scanType[scan]}")
                        if self.scanType[scan] == "xpcsscan":
                            # print dir(self.sd)
                            dataFile = curScan.XPCS["batch_name"][0]
                            self.immDataFileName[scan] = dataFile
                            logger.debug(f"curScan.XPCS {curScan.XPCS}")
                            if dataFile not in self.scanDataFile.keys():
                                self.scanDataFile[dataFile] = {}
                                numImagesInScan = self.imagesInScan(scan)
                                self.scanDataFile[dataFile][scan] = range(0, numImagesInScan)
                                self.scanDataFile[dataFile]["maxIndexImage"] = numImagesInScan
                                logger.debug(
                                    "scanDataFile for "
                                    + str(dataFile)
                                    + " at scan "
                                ...
                                startingImage = self.scanDataFile[dataFile]["maxIndexImage"]
                                numImagesInScan = self.imagesInScan(scan)
                                self.scanDataFile[dataFile][scan] = range(
                                    startingImage, startingImage + numImagesInScan
                                )
                                self.scanDataFile[dataFile]["maxIndexImage"] = startingImage + numImagesInScan
                                logger.debug(
                                    "scanDataFile for "
                                    + str(dataFile)
                                    + " at scan "
                        ...
                        elif curScan.CCD is not None and "ccdscan" in curScan.CCD.keys():
                            ...
                            if (self.pathToReplace is not None) and (self.replacePathWith is not None):
                                self.dataFilePrefix[scan] = self.dataFilePrefix[scan].replace(
                                    self.pathToReplace, self.replacePathWith
                                )
                            ...
                            logger.debug(f"isCcdScan[{scan}] {self.isCcdScan[scan]}")
                            logger.debug(f"dataFilePrefix[{scan}] {self.dataFilePrefix[scan]}")
                            if self.containsDark[scan]:
                                logger.debug(f"containsDark[{scan}] {self.containsDark[scan]}")
                            logger.debug(f"numberOfDarks[{scan}] {self.numberOfDarks[scan]}")
                            logger.debug(f"containsRoi[{scan}] {self.containsRoi[scan]}")
                            if self.containsRoi[scan]:
                                ...
                                logger.debug(f"roi[{scan}] {self.roi[scan]}")
                            logger.debug(f"repeatsPoints[{scan}] {self.repeatsPoints[scan]}")
                            if self.repeatsPoints[scan]:
                                logger.debug(f"numberOfRepeatPoints[{scan}] {self.numberOfRepeatPoints[scan]}")

                        if self.mapHKL:
                            self.ubMatrix[scan] = self.getUBMatrix(curScan)
                            if self.ubMatrix[scan] is None:
                                raise Sector33SpecFileException("UB matrix " + "not found.")
                        else:
    ...
    def rawmap(self, scans, mask=None):
        ...
        for scannr in scans:
            ...
            if curScan.CCD is not None and "ccdscan" in curScan.CCD.keys():
                ...
                if self.containsDark[scannr]:
                    ...
                    mask1 = np.array(mask)
                    logger.debug(f"mask1.shape {str(mask1.shape)}")
                    indexesToProcess = (np.where(mask1))[0]
                    logger.debug(f"indexesToProcess.shape {str(indexesToProcess.shape)}")
                    logger.debug(f"indexesToProcess {str(indexesToProcess)}")
                    ...
                    for scanno in indexesToProcess:
                        ...
                        fileName = namePrefix + "%d_%.5d-%.5d" % (scanno, 1, 1) + IMM_STR[1:]
                        logger.debug(f"Reading filename {fileName} fileName")
                        imageStartIndex, dlen = GetStartPositions(fileName, 1)
                        # read single image from file with one image
                        startImage = 0
                        numImages = 1
                        image = OpenMultiImm(fileName, startImage, numImages, imageStartIndex, dlen)[0]
                        if not arrayInitializedForScan:
                            if not intensity.shape[0]:
                                ...
                                intensity = np.concatenate(
                                    (intensity, (np.zeros((indexesToProcess.shape[0],) + image.shape))), axis=0
                                )
                        logger.debug(f"foundIndex {foundIndex}")
                        logger.debug(f" instensity.shape {str(intensity.shape)}")
                        logger.debug(f"image.shape {str(image.shape)}")
                        if self.containsDark[scannr]:
                ...
                try:
                    ...
                    fp.close()
                    imageStartIndex, dlen = GetStartPositions(fullFileName, numImages)
                    fp = open(fullFileName, "rb")
                    header = readHeader(fp, imageStartIndex[self.scanDataFile[immDataFile][scannr][0]])
                    self.detectorROI = [header["row_beg"], header["row_end"], header["col_beg"], header["col_end"]]
                except OSError as ex:
                    logger.error("Problem opening IMM file to get the start indexes" + str(ex))
                finally:
                    fp.close()
                if self.haltMap:
                ...
                imagesToSkip = self.imagesBeforeScan(scannr)
                ...
                images = OpenMultiImm(
                    fullFileName, imagesToSkip + startIndex - 1, numberToProcess, imageStartIndex, dlen
                )
                minorProgressInc = self.progressInc / indexesToProcess.shape[0]
                minorProgress = self.progress
        ...
        logger.debug(
            f"Shape of qxTrans {str(qxTrans.shape)}, qyTrans {str(qyTrans.shape)}, qzTrans {str(qzTrans.shape)}, intensity {str(intensity.shape)}"
        )
    ...
    def rawmapSingle(self, scan):
        ...
        for i in range(len(angleNames)):
            scanAngle[i] = np.concatenate((scanAngle[i], np.array(angles[:, i])), axis=0)
        # transform scan angles to reciprocal space coordinates for all detector
        # pixels
        angleList = []
        for i in range(len(angleNames)):
        ...
        if self.ubMatrix[scan] is None:
            ...
            qx, qy, qz = hxrd.Ang2Q.area(
                *angleList, roi=self.getDetectorROI(), Nav=self.getNumPixelsToAverage(), UB=self.ubMatrix[scan]
            )
