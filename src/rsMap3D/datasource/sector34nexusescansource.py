...
class Sector34NexusEscanSource(AbstractDataSource):
    ...
    def __init__(
        self, projectDir, projectName, projectExtension, detConfigFile, detectorId="PE1621 723-3335", **kwargs
    ):
        """ """
    ...
    def findImageQs(self):
        ...
        imageSize = self.getDetectorDimensions()[0] * self.getDetectorDimensions()[1]
        ...
        if imageSize * 4 * numImages <= maxImageMem:
            ...
            for thisPass in range(nPasses):
                ...
                (qx, qy, qz) = self.qsForDetector(startFile=firstImage, endFile=lastImage)
                idx = range(len(qx))
    ...
    def loadDetectorConfig(self):
        try:
            ...
            self.detectorDimensions = self.detConfig.getNpixels(self.detector)
            ...
            self.detectorTranslation = self.detConfig.getTranslation(self.detector)
            ...
            if theta == 0:
                ...
                self.rho2 = np.array(
                    [
                        [c + Rx * Rx * c1, Rz * s + Rx * Ry * c1, Ry * s + Rx * Rz * c1],
    ...
    def loadSource(self):
        ...
        if self.files is None:
            # Getting file list
            fileFilter = (
                str(os.path.join(self.projectDir, self.projectName.rsplit("_", 1)[0]))
                + "_[0-9]*"
        ...
        for afile in self.files:
            if self.cancelLoad:
                ...
                filename = (
                    os.path.join(self.projectDir, (self.projectName.rsplit("_", 1))[0])
                    + "_"
                    + str(afile)
                    + self.projectExtension
                )
                if os.path.exists(filename):
                    ...
                    try:
                        ...
                        self.incidentEnergy[afile] = hdfFile[H5_INCIDENT_ENERGY].value[0]
                        self.detectorROI = [
    ...
    def pixel2XYZ(self, pixelX, pixelY):
        xp = (pixelX - 0.5 * (self.detectorDimensions[0]) - 1) * self.detectorPixelWidth[0]
        yp = (pixelY - 0.5 * (self.detectorDimensions[1]) - 1) * self.detectorPixelWidth[1]

        xp += self.detectorTranslation[0]
        yp += self.detectorTranslation[1]
        zp = self.detectorTranslation[2]
        xyz = [
            (self.rho[0][0] * xp + self.rho[0][1] * yp + self.rho[0][2] * zp),
    ...
    def pixel2XYZ_2(self, indexMesh):
        ...
        zp = np.multiply(np.ones((self.detectorDimensions[0], self.detectorDimensions[1])), self.detectorTranslation[2])
    ...
    def qsForDetector(self, startFile=0, endFile=0):
        ...
        if (startFile == 0) and (endFile == 0):
            ...
            ((startFile >= 0) and (startFile <= len(self.availableFiles)))
            and ((endFile >= 0) and (endFile <= len(self.availableFiles)))
            and (startFile <= endFile)
        ):
            processFiles = self.availableFiles[startFile:endFile]
        else:
            raise ValueError(
                "startFile and EndFile must be valid file "
                + "indexes. StartFile = "
                + str(startFile)
                + "endFile = "
                + str(endFile)
                    ...
                    if not intensity.shape[0]:
                        ...
                        intensity = np.concatenate(
                            (
                                ...
                                (
                                    np.zeros(
                                        (np.count_nonzero(len(self.availableFiles[firstScan:lastScan])),) + img.shape
                                    )
                                ),
                            ),
                            axis=0,
