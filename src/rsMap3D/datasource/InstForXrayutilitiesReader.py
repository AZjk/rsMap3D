...
class InstForXrayutilitiesReader:
    ...
    def getInplaneReferenceDirection(self):
        ...
        """
        direction = self.root.find(INPLANE_REFERENCE_DIRECTION)
        if direction is None:
            raise InstConfigException("Missing in-plane reference direction " + "in instrument config file")
    ...
    def getPrimaryBeamDirection(self):
        ...
        """
        direction = self.root.find(PRIMARY_BEAM_DIRECTION)
        if direction is None:
            raise InstConfigException("Missing primary beam direction " + "in instrument config file")
        return self._makeReferenceDirection(direction)
    ...
    def getProjectionDirection(self):
        ...
        direction = self.root.find(PROJECTION_DIRECTION)
        if direction is None:
            message = PROJECTION_DIRECTION + " was not found in the instrument config file"
    ...
    def getSampleAngleMappingCalcOnScannedRef(self):
        ...
        if function is None:
            raise InstConfigException("No Mapping function defined in " + "instrument config file")
        if function.attrib[CALC_ON_SCANNED_REF] is None:
            return False
        else:
            if function.attrib[CALC_ON_SCANNED_REF].lower() in ["true", "1", "yes"]:
                return True
            elif function.attrib[CALC_ON_SCANNED_REF].lower() in ["false", "0", "no"]:
                return False
            else:
                raise InstConfigException(
    ...
    def getSampleAngleMappingPrimaryAngles(self):
        ...
        if primaryAngles is None:
            raise InstConfigException("No primaryAngle members in " + "sampleAngleMapFunction in instrument config")
        for angle in primaryAngles:
            angles.append(int(angle.attrib[AXIS_NUMBER]))
        angles.sort()
        return angles

    def getSampleAngleMappingReferenceAngleAttrib(self, number=None, attribName=None):
        attribValue = None
        function = self.root.find(SAMPLE_ANGLE_MAP_FUNCTION)
        if function is None:
            raise InstConfigException("No Mapping function defined in " + "instrument config file")
        ...
        if attribValue is None:
            raise (AttributeError("Angle " + str(number) + " does not have a value for " + str(attribName)))
        return attribValue
    ...
    def getSampleAngleMappingReferenceAngles(self):
        ...
        if referenceAngles is None:
            raise InstConfigException("No referenceAngle members in " + "sampleAngleMapFunction in instrument config")
        for angle in referenceAngles:
            angles[int(angle.attrib[AXIS_NUMBER])] = angle.attrib[SPEC_MOTOR_NAME]
    ...
    def getSampleCircles(self):
        ...
        if circles is None:
            raise InstConfigException("Instrument configuration has no Sample" + " Circles")
        return list(circles)
    ...
    def getSampleSurfaceNormalDirection(self):
        ...
        """
        direction = self.root.find(SAMPLE_SURFACE_NORMAL_DIRECTION)
        if direction is None:
            raise InstConfigException("Missing sample surface normal " + "direction in instrument config file")
        return self._makeReferenceDirection(direction)
