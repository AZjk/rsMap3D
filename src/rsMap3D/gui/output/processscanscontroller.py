...
class ProcessScansController(qtWidgets.QDialog):
    ...
    def __init__(self, parent=None, appConfig=None):
        ...
        self.outputFormWidget = self.outputForms[0].createInstance(appConfig=self.appConfig)
    ...
    def _connectSignals(self):
        self.outputFormSelection.currentTextChanged.connect(self._selectedTypeChanged)
    ...
    def _disconnectSignals(self):
        self.outputFormSelection.currentTextChanged.disconnect(self._selectedTypeChanged)
    ...
    def _processFormError(self, message):
        ...
        messageBox.warning(self, "Processing Scan File Warning", str(message))
    ...
    def runMapper(self):
        ...
        try:
            self.outputFormWidget.runMapper(self.parent.getDataSource(), self.parent.getTransform())
    ...
    def _selectedTypeChanged(self, typeStr):
        logger.debug("ProcessScanController::_SelectedType " + "Changed updating widget to " + str(typeStr))
        ...
        for form in self.outputForms:
            if typeStr == form.FORM_TITLE:
                ...
                self.outputFormWidget = form.createInstance(appConfig=self.appConfig)
    ...
    def _spawnProcessThread(self):
        ...
        self.setCancelOK()
        self.processThread = ProcessScanThread(
            self, self.parent.getDataSource(), self.parent.getTransform(), parent=None
        )
