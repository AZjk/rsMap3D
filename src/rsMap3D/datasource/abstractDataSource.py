...
class AbstractDataSource:
    ...
    def getOverallRanges(self):
        ...
        for scan in self.availableScans:
            overallXmin = min(overallXmin, np.min(self.imageBounds[scan][XMIN_INDEX]))
            overallXmax = max(overallXmax, np.max(self.imageBounds[scan][XMAX_INDEX]))
            overallYmin = min(overallYmin, np.min(self.imageBounds[scan][YMIN_INDEX]))
            overallYmax = max(overallYmax, np.max(self.imageBounds[scan][YMAX_INDEX]))
            overallZmin = min(overallZmin, np.min(self.imageBounds[scan][ZMIN_INDEX]))
            overallZmax = max(overallZmax, np.max(self.imageBounds[scan][ZMAX_INDEX]))

        return overallXmin, overallXmax, overallYmin, overallYmax, overallZmin, overallZmax
    ...
    def inBounds(self, xmin, xmax, ymin, ymax, zmin, zmax):
        ...
        return (
            (xmin <= self.rangeBounds[XMAX_INDEX] and xmax >= self.rangeBounds[XMIN_INDEX])
            and (ymin <= self.rangeBounds[YMAX_INDEX] and ymax >= self.rangeBounds[YMIN_INDEX])
            and (zmin <= self.rangeBounds[ZMAX_INDEX] and zmax >= self.rangeBounds[ZMIN_INDEX])
        )
