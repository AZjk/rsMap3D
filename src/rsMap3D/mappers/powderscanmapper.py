...
class PowderScanMapper:
    ...
    def getFileInfo(self):
        ...
        return (
            str(os.path.join(self.dataSource.projectDir, self.dataSource.projectName)),
            self.currentMapScans[0],
            numBins,
            self.outputFileName,
            self.dataCoord,
            self.yScaling,
        )  # ===== carry over the xlabel and y scanling, ZZ
    ...
    def getXCoordMax(self):
        ...
        if self.xCoordMax is not None and not (self.xCoordMax == EMPTY_STR):
            xMax = float(self.xCoordMax)
        else:
            qxMin, qxMax, qMin, qyMax, qzMin, qzMax = self.dataSource.getRangeBounds()
            ...
            if self.dataCoord == X_COORD_OPTIONS[0]:
                xMax = np.rad2deg(np.arcsin((QMax * wavelen) / (4.0 * np.pi)) * 2.0)
            else:
                xMax = QMax
        logger.debug(f"xMax {xMax:f}")
        return xMax
    ...
    def getXCoordMin(self):
        ...
        scans = self.currentMapScans
        wavelen = 12398.41290 / self.dataSource.getIncidentEnergy()[scans[0]]
        if self.xCoordMin is not None and not (self.xCoordMin == EMPTY_STR):
            xMin = float(self.xCoordMin)
        else:
            qxMin, qxMax, qyMin, qyMax, qzMin, qzMax = self.dataSource.getRangeBounds()
            absQxMin = None
            ...
            if self.dataCoord == X_COORD_OPTIONS[0]:
                ...
                #                              (QMin, wavelen, ))
                xMin = np.rad2deg(np.arcsin((QMin * wavelen) / (4.0 * np.pi)) * 2.0)
            else:
                xMin = QMin
        logger.debug(f"xMin {xMin:f}")
        return xMin
    ...
    def processMap(self):
        ...
        for scan in self.currentMapScans:
            ...
            logger.info(f"Scan Number {scan}")
            numImages = len(imageToBeUsed[scan])
            if imageSize * 4 * numImages <= maxImageMem:
                logger.info("Only 1 pass required")
                qx, qy, qz, intensity = self.dataSource.rawmap((scan,), mask=imageToBeUsed[scan])
                ...
                for thisPass in range(nPasses):
                    ...
                    qx, qy, qz, intensity = self.dataSource.rawmap((scan,), mask=imageToBeUsedInPass)
                    Q = np.sqrt(qx**2 + qy**2 + qz**2)
                    coordsX = None
                    if self.dataCoord == X_COORD_OPTIONS[0]:
                        coordsX = np.rad2deg(np.arcsin((Q * wavelen) / (4.0 * np.pi)) * 2.0)
                    ...
                    if self.progressUpdater is not None:
                        self.progressUpdater(int(progress))
                    # =====
        err = np.where(gridder._gnorm > 0.0, np.sqrt(gridder._gdata) / gridder._gnorm, 0.0)
