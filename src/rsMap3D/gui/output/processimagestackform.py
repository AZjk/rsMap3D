...
from rsMap3D.gui.rsm3dcommonstrings import BINARY_OUTPUT, BROWSE_STR, SAVE_DIR_STR, WARNING_STR
...
class ProcessImageStackForm(AbstractGridOutputForm):
    ...
    def _browseForOutputDirectory(self):
        ...
        if self.outputDirTxt.text() == "":
            dirName = str(qtWidgets.QFileDialog.getExistingDirectory(None, SAVE_DIR_STR)[0])
        else:
            curName = str(self.outputDirTxt.text())
            dirName = str(qtWidgets.QFileDialog.getExistingDirectory(None, SAVE_DIR_STR, directory=curName)[0])
        ...
        if dirName != "":
            ...
            if not os.access(dirName, os.W_OK):
                message = qtWidgets.QMessageBox()
                message.warning(self, WARNING_STR, "The specified directory is not writable")
    ...
    def _createDataBox(self):
        ...
        self.imageFilePrefixTxt.editingFinished.connect(self._editFinishedOutputDir)
        ...
        if dirName != "":
            if os.path.exists(dirName):  # use specified dir if it exists
                ...
                if dirName == "":
                    ...
                    message = qtWidgets.QMessageBox()
                    message.warning(self, WARNING_STR, "The specified directory \n" + str(dirName) + "\ndoes not exist")
            ...
            for badChar in ["\\", "/", ":", "*", "?", "<", ">", "|"]:
                if badChar in imageFilePrefix:
                    ...
                    message.warning(
                        self,
                        WARNING_STR,
                        "The specified imagePrefix conatins one " + "of the following invalid characters \\/:*?<>|",
                    )
            else:
