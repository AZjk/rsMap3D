...
class S12SpecScanFileForm(SpecXMLDrivenFileForm):
    ...
    def _badPixelFileChanged(self):
        ...
        """
        logger.debug(METHOD_ENTER_STR)
        if os.path.isfile(self.badPixelFileTxt.text()) or self.badPixelFileTxt.text() == EMPTY_STR:
            self.checkOkToLoad()
        else:
            message = qtWidgets.QMessageBox()
            message.warning(self, WARNING_STR, "The filename entered for the bad pixel " + "file is invalid")
        logger.debug(METHOD_EXIT_STR)
    ...
    def _browseBadPixelFileName(self):
        ...
        if self.badPixelFileTxt.text() == EMPTY_STR:
            fileName = qtWidgets.QFileDialog.getOpenFileName(
                None, SELECT_BAD_PIXEL_TITLE, filter=BAD_PIXEL_FILE_FILTER
            )[0]
        else:
            fileDirectory = os.path.dirname(str(self.badPixelFileTxt.text()))
            fileName = qtWidgets.QFileDialog.getOpenFileName(
                None, SELECT_BAD_PIXEL_TITLE, filter=BAD_PIXEL_FILE_FILTER, directory=fileDirectory
            )[0]
    ...
    def _browseFlatFieldFileName(self):
        ...
        if self.flatFieldFileTxt.text() == EMPTY_STR:
            fileName = qtWidgets.QFileDialog.getOpenFileName(None, SELECT_FLAT_FIELD_TITLE, filter=TIFF_FILE_FILTER)[0]
        else:
            fileDirectory = os.path.dirname(str(self.flatFieldFileTxt.text()))
            fileName = qtWidgets.QFileDialog.getOpenFileName(
                None, SELECT_FLAT_FIELD_TITLE, filter=TIFF_FILE_FILTER, directory=fileDirectory
            )[0]
        if fileName != EMPTY_STR:
    ...
    def checkOkToLoad(self):
        ...
        retVal = False
        if (
            os.path.isfile(self.projNameTxt.text())
            and os.path.isfile(self.instConfigTxt.text())
            and os.path.isfile(self.detConfigTxt.text())
            and (
                self.noFieldRadio.isChecked()
                or (self.badPixelRadio.isChecked() and not (str(self.badPixelFileTxt.text()) == ""))
                or (self.flatFieldRadio.isChecked() and not (str(self.flatFieldFileTxt.text()) == ""))
            )
            and self.pixAvgValid(self.pixAvgTxt.text())
            and self.detROIValid(self.detROITxt.text())
        ):
            retVal = True
    ...
    def _createControlBox(self):
        ...
        """
        logger.debug(METHOD_ENTER_STR)
        controlBox = super()._createControlBox()
        logger.debug(METHOD_EXIT_STR)
        return controlBox
    ...
    def _createDataBox(self):
        ...
        self.fieldCorrectionGroup.buttonClicked.connect(self._fieldCorrectionTypeChanged)

        self.badPixelFileTxt.editingFinished.connect(self._badPixelFileChanged)
        self.badPixelFileBrowseButton.clicked.connect(self._browseBadPixelFileName)

        self.flatFieldFileTxt.editingFinished.connect(self._flatFieldFileChanged)
        self.flatFieldFileBrowseButton.clicked.connect(self._browseFlatFieldFileName)
    ...
    def _flatFieldFileChanged(self):
        ...
        """
        logger.debug(METHOD_ENTER_STR)
        if os.path.isfile(self.flatFieldFileTxt.text()) or self.flatFieldFileTxt.text() == "":
            self.checkOkToLoad()
        else:
            message = qtWidgets.QMessageBox()
            message.warning(self, WARNING_STR, "The filename entered for the flat field " + "file is invalid")
        logger.debug(METHOD_EXIT_STR)
    ...
    def getBadPixelFileName(self):
        ...
        logger.debug(METHOD_ENTER_STR)
        retVal = None
        if (str(self.badPixelFileTxt.text()) == EMPTY_STR) or (not self.badPixelRadio.isChecked()):
            retVal = None
        else:
    ...
    def getDataSource(self):
        ...
        self.dataSource = Sector12SpecDataSource(
            ...
            pixelsToAverage=self.getPixelsToAverage(),
            badPixelFile=self.getBadPixelFileName(),
            flatFieldFile=self.getFlatFieldFileName(),
    ...
    def getFlatFieldFileName(self):
        ...
        logger.debug(METHOD_ENTER_STR)
        retVal = None
        if (str(self.flatFieldFileTxt.text()) == EMPTY_STR) or (not self.flatFieldRadio.isChecked()):
            retVal = None
        else:
