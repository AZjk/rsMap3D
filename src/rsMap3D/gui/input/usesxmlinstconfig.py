...
class UsesXMLInstConfig(AbstractFileView):
    ...
    def _browseForInstFile(self, checked):
        ...
        if self.instConfigTxt.text() == EMPTY_STR:
            fileName = qtWidgets.QFileDialog.getOpenFileName(
                None, SELECT_INSTRUMENT_CONFIG_TITLE, filter=INSTRUMENT_CONFIG_FILE_FILTER
            )[0]
        else:
            fileDirectory = os.path.dirname(str(self.instConfigTxt.text()))
            fileName = qtWidgets.QFileDialog.getOpenFileName(
                None, SELECT_INSTRUMENT_CONFIG_TITLE, filter=INSTRUMENT_CONFIG_FILE_FILTER, directory=fileDirectory
            )[0]
    ...
    def _instConfigChanged(self):
        ...
        if self.instFileExists() or self.instConfigTxt.text() == EMPTY_STR:
            self.checkOkToLoad()
            if self.instConfigTxt.text() != EMPTY_STR:
                try:
                    ...
                    message.warning(
                        self,
                        WARNING_STR,
                        "Trouble getting the projection direction " + "from the instrument config file.",
                    )
        else:
            message = qtWidgets.QMessageBox()
            message.warning(self, WARNING_STR, "The filename entered for the instrument " + "configuration is invalid")
        logger.debug(METHOD_EXIT_STR)
    ...
    def updateProjectionDirection(self):
        ...
        """
        logger.debug(METHOD_ENTER_STR)
        instConfig = InstForXrayutilitiesReader(self.instConfigTxt.text())
