...
class RSMap3DConfig:
    ...
    def __init__(self, fileName="", exeptionOnNoFile=False):
        if fileName == "":
            ...
            logger.debug("No filename given so using default name " + str(self.configFile))
        else:
            self.configFile = fileName
            logger.debug("Filename passed as argument using that " + str(self.configFile))
        try:
    ...
    def getMaxImageMemory(self):
        ...
        if maxIMem is None:
            raise RSMap3DException("Config file " + self.configFile + " does not define maxImageMemory")
        try:
