...
class Test(unittest.TestCase):
    ...
    def testGetDetectorByID(self):
        ...
        self.assertEqual(config.getDetectorID(det1), DETECTOR_NAME1, "Detector 1 returnend the wrong name")
        det2 = config.getDetectorById(DETECTOR_NAME2)
        self.assertNotEqual(det2, None, "Trouble finding detector 2")
        self.assertEqual(config.getDetectorID(det2), DETECTOR_NAME2, "Detector 2 returnend the wrong name")
        det3 = config.getDetectorById(DETECTOR_NAME3)
        self.assertNotEqual(det3, None, "Trouble finding detector 3")
        self.assertEqual(config.getDetectorID(det3), DETECTOR_NAME3, "Detector 3 returnend the wrong name")
        self.assertRaises(DetectorConfigException, config.getDetectorById, DUMMY_DETECTOR)
    ...
    def testGetNPixels(self):
        ...
        self.assertNotEqual(det1, None, "Trouble finding detector 1")
        self.assertEqual(
            config.getNpixels(det1),
            BIG_DETECTOR_PIXELS,
            "Detector 1 returnend the number of pixels" + str(config.getNpixels(det1)),
        )
        det2 = config.getDetectorById(DETECTOR_NAME2)
        self.assertNotEqual(det2, None, "Trouble finding detector 2")
        self.assertEqual(
            config.getNpixels(det2),
            SMALL_DETECTOR_PIXELS,
            "Detector 2 returnend the number of pixels" + str(config.getNpixels(det2)),
        )
        det3 = config.getDetectorById(DETECTOR_NAME3)
        self.assertNotEqual(det3, None, "Trouble finding detector 3")
        self.assertEqual(
            config.getNpixels(det3),
            SMALL_DETECTOR_PIXELS,
            "Detector 3 returnend the wrong number of pixels" + str(config.getNpixels(det3)),
        )
        self.assertRaises(DetectorConfigException, config.getDetectorById, DUMMY_DETECTOR)
    ...
    def testGetSize(self):
        ...
        self.assertNotEqual(det1, None, "Trouble finding detector 1")
        self.assertEqual(
            config.getSize(det1), BIG_DETECTOR_SIZE, "Detector 1 returnend the wrong size" + str(config.getSize(det1))
        )
        det2 = config.getDetectorById(DETECTOR_NAME2)
        self.assertNotEqual(det2, None, "Trouble finding detector 2")
        self.assertEqual(
            config.getSize(det2), SMALL_DETECTOR_SIZE, "Detector 2 returnend the wrong size" + str(config.getSize(det2))
        )
        det3 = config.getDetectorById(DETECTOR_NAME3)
        self.assertNotEqual(det3, None, "Trouble finding detector 3")
        self.assertEqual(
            config.getSize(det3), SMALL_DETECTOR_SIZE, "Detector 3 returnend the wrong size" + str(config.getSize(det3))
        )
        self.assertRaises(DetectorConfigException, config.getDetectorById, DUMMY_DETECTOR)
    ...
    def testGetTranslation(self):
        ...
        self.assertNotEqual(det1, None, "Trouble finding detector 1")
        self.assertEqual(
            config.getTranslation(det1),
            DETECTOR_TRANSLATION1,
            "Detector 1 returnend the wrong translation" + str(config.getTranslation(det1)),
        )
        det2 = config.getDetectorById(DETECTOR_NAME2)
        self.assertNotEqual(det2, None, "Trouble finding detector 2")
        self.assertEqual(
            config.getTranslation(det2),
            DETECTOR_TRANSLATION2,
            "Detector 2 returnend the wrong translation" + str(config.getTranslation(det2)),
        )
        det3 = config.getDetectorById(DETECTOR_NAME3)
        self.assertNotEqual(det3, None, "Trouble finding detector 3")
        self.assertEqual(
            config.getTranslation(det3),
            DETECTOR_TRANSLATION3,
            "Detector 3 returnend the wrong TRANSLATION" + str(config.getTranslation(det3)),
        )
        self.assertRaises(DetectorConfigException, config.getDetectorById, DUMMY_DETECTOR)
